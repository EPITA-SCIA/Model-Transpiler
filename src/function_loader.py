...
class FunctionLoader:
    ...
    def _get_function_map(self, directory: Path) -> dict:
        function_map = {"c": dict(), "verilog": dict()}
        c_dir = directory / "c"
        verilog_dir = directory / "verilog"

        for file in c_dir.glob("*.c"):
            function_name = file.stem
            function_map["c"][function_name] = self._load_file(file)

        for file in verilog_dir.glob("*.v"):
            function_name = file.stem
            function_map["verilog"][function_name] = self._load_file(file)
        return function_map
